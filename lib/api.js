 ...
 * @param {boolean} [countOnly=false] - If true, returns only the count of favourites
 * @returns {Promise<Object>} - A promise that resolves to favourites data or count
 */
export async function fetchFavourites(token, countOnly = false) {
  try {
    ...
    return { error: `Error fetching favourites: ${error.message}` };
  }
}
