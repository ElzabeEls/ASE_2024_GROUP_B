 ...
 * @param {boolean} [countOnly=false] - If true, returns only the count of favorites
 * @returns {Promise<Object>} - A promise that resolves to favorites data or count
 */
export async function fetchFavorites(token, countOnly = false) {
  try {
    const url = new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/api/favourites`);
    if (countOnly) {
      url.searchParams.append("action", "count");
    }
    ...
    const data = await response.json();
    return countOnly ? data.count : data.favourites;
  } catch (error) {
 ...
 * @param {string} token - JWT token for authentication
 * @param {string} recipeId - ID of the recipe to add to favorites
 * @returns {Promise<Object>} - A promise that resolves to the response message
 */
export async function addToFavorites(token, recipeId) {
  try {
    ...
    return { error: `Error adding to favorites: ${error.message}` };
  }
}
 ...
 * @param {string} token - JWT token for authentication
 * @param {string} recipeId - ID of the recipe to remove from favorites
 * @returns {Promise<Object>} - A promise that resolves to the response message
 */
export async function removeFromFavorites(token, recipeId) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/favourites`, {
      method: 'DELETE',
      headers: {
    ...
    return { error: `Error removing from favorites: ${error.message}` };
  }
}